...
public class BookService {
    ...
    private final BookDAO bookDAO; // final이기 때문에 setter 방식을 쓸 수 없다.
    ...
    * 단, 생성자가 1개 이상일 경우에는 명시적으로 작성해주어야 한다.
    ...
    * - 객체의 불변성을 보장할 수 있다.
    *   - 필드에 'final' 사용이 가능하고 객체 생성 이후 의존성을 변경할 수 없어 안정성이 보장된다.
    * - 코드 가독성이 좋다.
    *   - 해당 객체가 어떤 의존성을 가지고 있는지 명확히 알 수 있다.
    * - DI 컨테이너와 결합도가 낮기 때문에 테스트하기 좋다.
    *   - 스프링 컨테이너 없이 테스트할 수 있다.
    * */
