...
public class Application {
    ...
    /* 리플렉션이란?
    * 컴파일되 자바 코드에서 역으로 클래스를 불러 메소드 및 필드 정보를 구해오는 방법으로 반사, 투영이라는 의미를 가진다.
...
//        Class class8 = d.getClass();
        ...
        * */
        ...
        try {
            ...
            // constructors[0]는 기본 생성자이기 때문에 받지 못한다.
        ...
        Method getBalnceMethod = null;
        for(Method method : methods){
            System.out.println(Modifier.toString(method.getModifiers()) + " " + method.getReturnType().getSimpleName() + " " + method.getName());

            if("getBalance".equals(method.getName())){
                getBalnceMethod = method;
            }
        }

        try {
            System.out.println(getBalnceMethod.invoke((Account)constructors[0].newInstance()));
        } catch (IllegalAccessException e) {
            throw new RuntimeException(e);
        } catch (InvocationTargetException e) {
            throw new RuntimeException(e);
