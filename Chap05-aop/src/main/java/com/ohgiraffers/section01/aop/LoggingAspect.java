...
/*
* pointcut : 특정 조건에 의해 필터링된 조인 포인트, 수 많은 조인 포인트 중에 특정 메소드만 횡단 공통기능을 수행하기 위해서 사용한다.
* advice : 횡단 관심에 해당하는 공통 기능의 코드, 독립된 클래스의 메소드로 작성한다.
*   - before : 메소드 실행 전에 동작
*   - After : 메소드 실행 후에 동작
*   - After-returning : 메소드가 정상적으로 실행된 후에 동작
*   - After-throwing : 예외가 발생한 후에 동작
*   - Around : 메소드 호출 이전, 이후, 예외 발생 등 모든 시점에 동작*/
@Aspect
@Component
public class LoggingAspect {
    ...
    * */
    ...
    // *Service.*(..) : 매개변수가 0개 이상은 모든 메소드
    // *Service.*(*) : 매개변수가 1개인 모든 메소드
    // *Service.*(*, ..) : 매개변수가 2개인 모든 메소드
    @Pointcut("execution(* com.ohgiraffers.section01.aop.*Service.*(..))") // ~Service에 있는 *(..)(모든 메소드)을 대상으로
    ...
    * 매개변수로 전달한 joinPoint 객체는 현재 조인 포인트의 메소드명, 인수값 등의 자세한 정보를 액세스할 수 있다.
    * */
    @Before("LoggingAspect.logPointcut()")
    public void logBefore(JoinPoint joinPoint){
        System.out.println("before joinPoint.getTarget() " + joinPoint.getTarget());
        System.out.println("Before joinPoint.getSignature() " + joinPoint.getSignature());
        if(joinPoint.getArgs().length > 0){ // 전달받은 매개변수가 있다면
    ...
    * */
    ...
    /* aop가 적용될 메소드가 에러를 발생하여 Exception을 던지는 시점에 동작한다. */
    ...
    * 최소한 요건을 충족하면서도 가장 기능이 약한 어드바이스를 쓰는게 바람직하다.
    * */
    @Around("logPointcut()")
